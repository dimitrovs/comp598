...
clf = svm.SVC()
train_inputs_normalized = preprocessing.binarize(train_inputs, threshold=0.79)
test_inputs_normalized = preprocessing.binarize(test_inputs, threshold=0.79)
pca = PCA(n_components=100).fit(train_inputs_normalized, train_outputs)
train_inputs_new = pca.transform(train_inputs_normalized)
test_inputs_new = pca.transform(test_inputs_normalized)
...
test_output_file = open('test_output_svm_pca_100_norm.csv', "wb")
