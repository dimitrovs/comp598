...
parameters = {'kernel':('linear', 'poly', 'rbf', 'sigmoid'), 'C':[0.5, 1, 10,100]}
...
svr = svm.SVC()
clf = grid_search.GridSearchCV(svr, parameters,cv=10,verbose=True)
clf.fit(train_inputs_new[:1000], train_outputs[:1000])
