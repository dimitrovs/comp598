...
for threshold_param in range(1,100):
	train_inputs_normalized = preprocessing.binarize(train_inputs, threshold=(threshold_param/100.0))
	components = 100
	while components < 2000:
        	clf = GaussianNB()
        	pca = PCA(n_components=components)
        	train_inputs_new = pca.fit_transform(train_inputs_normalized,train_outputs)
        	print "PCA NB Norm Shape: {0}".format(len(train_inputs_new[0]))
        	scores = cross_validation.cross_val_score(clf, train_inputs_new, train_outputs, cv=10,n_jobs=1)
        	print("Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
        	components += 100
	#X_train, X_test, y_train, y_test = cross_validation.train_test_split(train_inputs_normalized, train_outpus, test_size=0.33, random_state=42)
	#clf.fit(X_train,y_train)
	#score = clf.score(X_test,y_test)
	#print("Threshold: %0.2f Accuracy: %0.4f" % (threshold_param/100.0, score))
	#scores = cross_validation.cross_val_score(clf, train_inputs_normalized, train_outputs, cv=10,n_jobs=10)
	#print("Threshold: %0.2f Accuracy: %0.4f (+/- %0.2f)" % (threshold_param/100.0, scores.mean(), scores.std() * 2))
